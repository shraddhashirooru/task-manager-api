...
A simple Task Management REST API built with FastAPI and PostgreSQL. Users can create, read, update, and delete tasks, with authentication using JWT tokens.
...
- Users log in with their username and password via the /login endpoint.

- After login, a JWT token is issued, which must be included in requests to authenticated routes (like creating, updating, or deleting tasks).
